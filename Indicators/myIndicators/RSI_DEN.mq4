...
//|                                              http://www.mql4.com |
//+------------------------------------------------------------------+
#property description "Relative Strength Index on Denoise"
#property strict
...
//--- name for DataWindow and indicator subwindow label
   short_name="RSI_DEN("+string(DenoiseSize)+","+string(InpRSIPeriod)+")";
...
int OnCalculate(const int rates_total,
                ...
                const int &spread[])
  {
   ...
   double diff;
//---
   if(Bars<=InpRSIPeriod+DenoiseSize || InpRSIPeriod+DenoiseSize<2)
      return(0);
...
//--- the main loop of calculations
   for(i=pos; i<rates_total && !IsStopped(); i++)
     {
      diff=iCustom(Symbol(), Period(),"myIndicators/DeNoise",DenoiseSize,0,i)
         -iCustom(Symbol(), Period(),"myIndicators/DeNoise",DenoiseSize,0,i-1);
      //diff=close[i]-close[i-1];
